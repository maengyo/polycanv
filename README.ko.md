...
컨셉은 **[cate](https://github.com/0-AI-UG/cate)** 에서 왔습니다 — *병렬 코딩 에이전트를 위한
무한 캔버스 IDE*. cate 는 캔버스를 mission control 로 만듭니다. 터미널마다 에이전트가 작업
중인지·끝났는지·기다리는지 보이고, git worktree 마다 **자기 색깔의 영역**을 가져서 다섯
브랜치의 다섯 에이전트가 탭 더미가 아니라 **분리된 다섯 개의 작업 흐름**으로 읽힙니다.

polycanv 는 그 발상을 터미널로 가져온 것입니다. 전제는 같습니다 — **세션에는 탭 번호가 아니라
자리가 필요하다.** 다만 범위를 의도적으로 좁혔습니다: 편집기도, 브라우저도, 데스크톱 앱도 없습니다.
공간형 IDE 전체가 필요하시면 cate 를 쓰세요. 그쪽이 더 풍부한 도구입니다.
