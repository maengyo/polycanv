...
- **신호등.** 터미널마다 제목 줄에 불이 하나 있다. 초록은 도는 중, 호박은 답을 기다리는
  중, 빨강은 끝남, 회색은 쉬는 중. 빨강은 그 터미널을 실제로 들여다보면 꺼지고,
  **호박은 안 꺼진다** — 승인 프롬프트는 쳐다본다고 사라지지 않기 때문이다.
...
- **남은 CLI 의 신호등** — claude code 와 qwen 은 지금 된다. codex 는 훅 신뢰 게이트를
  넘겨야 하고(#2), opencode 는 훅이 아니라 HTTP SSE 로 알려 준다(#4).
