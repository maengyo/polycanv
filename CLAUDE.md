...
| 배치 | 무한 줌 캔버스 (자유 좌표) | **자유 배치 캔버스** (좌표·크기를 각자 가짐) |
| 편집기·브라우저 | 포함 | **없음** — 터미널만 |

### 아직 못 따라간 것

cate 의 **"worktree 마다 색깔 있는 영역"** 이 우리에게 없다. 자리는 생겼지만(자유 캔버스)
**묶음**이 없다 — 같은 프로젝트에서 도는 터미널 둘과 다른 프로젝트의 터미널 둘이 똑같아 보인다.
→ 이슈 #13.
...
## 확정된 설계 결정 — 파이썬 빌드 (2026-08-20 실측)

### 키는 기본적으로 안쪽 프로그램의 것이다

**앱 단축키를 `ctrl+n` 같은 키에 두면 안 된다.** 전부 readline 이 쓴다 — `ctrl+w` 단어 삭제,
`ctrl+n` 히스토리, `ctrl+t` 글자 바꾸기, `ctrl+q` 흐름 제어. 안에서 도는 CLI 가 멀쩡히 쓰던
조작을 뺏는 셈이다. AI CLI 는 특히 `ctrl+c` 로 도는 턴을 끊는 일이 잦다.

→ **접두키 하나(`ctrl+b`)만 가져간다.** tmux 와 같은 키다. 두 번 누르면 그대로 안으로 보낸다.

구현 주의: 포커스를 가진 위젯이 키를 먼저 본다. 그래서 `TerminalPanel.on_key` 가 접두키와
그다음 한 글자를 **양보해야** 앱까지 닿는다.

### Textual 의 `event.character` 만으로는 터미널이 안 된다

실측: `ctrl+c`·`enter`·화살표 모두 `character` 가 `None` 이다. 이 값만 보고 PTY 에 쓰면
**명령 하나 실행할 수 없다.** `backspace` 는 `\x08` 로 오는데 유닉스는 `\x7f` 를 기대하고,
`delete` 는 반대로 `\x7f` 로 온다. → `keymap.py` 에서 이름으로 직접 옮긴다(xterm 기준).

### Textual 이 먼저 가져가는 키가 있다

- `ctrl+q` → App 의 **priority 바인딩**이라 위젯보다 먼저 잡힌다. 같은 키를 덮어써서 안으로 돌려보낸다.
- `ctrl+p` → 명령 팔레트. `ENABLE_COMMAND_PALETTE = False` 로 끈다.

### 패널은 캔버스 밖으로 나가면 안 된다

자유 배치는 **아무 데나 둘 수 있다**는 뜻이지 **잃어버릴 수 있다**는 뜻이 아니다.
끌다가 놓친 터미널을 되찾을 방법이 없다. 이동·크기 모두 캔버스 안으로 제한한다.

### 설정 파일은 어떤 경우에도 예외를 내지 않는다

`tools.toml` 이 깨졌다고 도구가 안 뜨면 사용자는 polycanv 가 고장 난 줄 안다.
못 읽으면 기본값으로 버티고 사유를 알린다. 항목 하나가 잘못됐으면 그것만 건너뛴다.

### 화면 확인 (에이전트가 TUI 를 볼 수 없다는 제약)

PTY 마스터가 받는 바이트가 곧 화면이다. 적어 두고 pyte 로 재생하면 확인할 수 있다.
이 방법으로 잡은 것: 접두키 미도달, `enter` 미전달, 패널 이탈.

**키를 붙여 쓰면 안 된다** — `\x02n` 을 한 번에 써 넣으면 앱이 한 덩어리로 읽어 접두키가
먹지 않는다. 한 글자씩 띄워 보내야 사람이 친 것과 같아진다(실측).

### 데모 녹화는 환경을 씻고 찍는다

그냥 찍으면 프롬프트에 **사용자명·호스트명·실제 경로**가 그대로 들어가고 공개 저장소의
이미지로 남는다. `HOME`·`SHELL`·`XDG_CONFIG_HOME` 을 임시 디렉터리로 돌리고 프롬프트를
고정한다.


## 성능 — 측정된 것과 그 값 (2026-08-20)

`scripts/dev/bench.py` 가 셸만·tmux·polycanv 를 같은 조건으로 잰다. **비교 없이 재지 마라** —
"40MB" 는 그 자체로 아무 뜻이 없다.

**메모리는 고정비다.** Textual 빈 앱이 이미 28.1MB 다(실측). polycanv 는 터미널 1개에 54.9MB,
8개에 74.8MB — **터미널당 2~3MB** 다. 많이 띄울수록 tmux(4.8 → 25.3MB)와 좁혀진다.
기준점: 이 기계의 Terminal.app 은 혼자 92MB 였다.

**CPU 는 부하에서 tmux 의 8배다** (`seq 1 200000`: 1.50초 vs 0.18초). 파이썬 에뮬레이터를
쓰는 값이다. **렌더링 탓이 아니다** — 렌더를 통째로 들어내도 1.50 → 1.47초다.
Textual 이 프레임 단위로 합쳐 주기 때문이다. 최적화하려면 pyte 자체를 건드려야 한다.

### 성능에서 되풀이된 함정

- **`pyte.Screen.display` 는 화면 전체를 새로 만드는 속성이다.** 한 줄이 필요해서 부르면
  12배가 느려진다(실측 0.13 → 1.40 MB/s). 줄 단위로 필요하면 직접 만들어라.
- **pyte 의 행은 성긴 dict 다.** `range(columns)` 로 훑지 말고 쓰인 데까지만 봐라.
- **패널마다 타이머를 걸지 마라.** 초당 20번 × 터미널 수가 유휴에 그대로 쌓인다.
  `asyncio` 의 `add_reader` 로 **읽을 게 생겼을 때만** 깨어난다.
- **읽기는 몰아서, 단 한도를 두고.** 한도가 없으면 끝없이 뱉는 프로그램 하나가
  이벤트 루프를 붙잡아 앱 전체(다른 터미널·키 입력)를 멈춘다.

### 벤치마크가 거짓말하는 방법 (셋 다 실제로 겪었다)

숫자가 그럴듯하게 나오기 때문에 더 위험하다.

- `ps -o rss= <pid들>` — **`-p` 를 빼면 인자가 무시되고 머신 전체가 나온다.**
  4개를 물었는데 93개를 더해 셸 4개가 2.6GB로 보였다.
- **tmux 는 전용 소켓(`-L`)으로 띄워라.** 안 그러면 사용자가 이미 돌리던 서버에 붙어
  남의 세션까지 잰다.
- **부하를 잴 때 배수 루프에서 잠들지 마라.** PTY 버퍼가 차서 자식이 쓰기에서 멈추고,
  재는 대상이 상대가 아니라 이쪽 루프가 된다(셸만 재는데 38.61초 → `select` 로 0.57초).
